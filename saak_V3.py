        ...
        if b_active and entry_p is not None:
            ...
            with lock:
                if not state["tp_triggered"]:
                    is_tp_touched = (direction == "BUY" and tick.high >= tp_price) or (direction == "SELL" and tick.low <= tp_price)
                    if is_tp_touched:
            ...
            if n >= 3:
                if not be_touched and pnl >= 0:
                    with lock:
                        state["g2_be_touched"] = True
                        be_touched = True
                    add_log("G3 breakeven hit. Trailing -$100 and Candle-End exit active.", "warn")
                
                if be_touched:
                    if pnl <= -100.0:
                        liquidate("G3 TRAILING SL (-$100)", False)
                        continue
            ...
            # Immediate Profit Target (Optional G2 Target if you want immediate exit)
