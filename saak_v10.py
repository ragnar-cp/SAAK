        ...
        if b_active and entry_p is not None:
            ...
            if n >= 3:
                if not be_touched and pnl >= 5.0:
                    with lock:
                        ...
                        be_touched = True
                    add_log(f"Level {n} breakeven hit (+$5). Trailing -$900 and Candle-End exit active.", "warn")
                
                if be_touched:
                    if pnl <= -900.0:
                        liquidate(f"Level {n} TRAILING SL (-$900)", False)
                        continue
            ...
            if new_candle_started:
                if tp_latched and pnl > 0:
                    ...
                    continue
                if n >= 3 and be_touched:
                    liquidate(f"Level {n} CANDLE END EXIT", pnl >= 0)
                    continue
