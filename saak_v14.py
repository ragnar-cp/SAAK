...
def send_order(direction, lot, comment="GRID"):
    ...
    for mode in FILLING_PRIORITY:
        ...
        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
            # --- LIVE-ACCOUNT FIX ---
            # On ECN/Market-Execution brokers (like Vantage Live), res.price
            # often returns 0.0 because the fill is confirmed asynchronously.
            # We MUST resolve the actual fill price reliably.
            fill_price = res.price
            if fill_price <= 0:
                # Strategy 1: Query the position that was just opened
                time.sleep(0.05)  # 50ms for broker to register the position
                positions = mt5.positions_get(symbol=SYMBOL)
                if positions:
                    # Find the position matching our order (by ticket or most recent)
                    for p in positions:
                        if p.magic == MAGIC and p.ticket == res.order:
                            fill_price = p.price_open
                            break
                    # Fallback: if ticket match fails, find the newest position
                    if fill_price <= 0:
                        our_pos = [p for p in positions if p.magic == MAGIC]
                        if our_pos:
                            fill_price = our_pos[-1].price_open
            if fill_price <= 0:
                # Strategy 2: Last resort - use the requested price
                fill_price = price
                add_log(f"WARN: Could not resolve fill price, using requested {price:.2f}", "warn")
            add_log(f"FILL {direction} {lot}L @ {fill_price:.2f} (mode={mode})", "buy" if direction=="BUY" else "sell")
...
def bot_thread():
    ...
    while True:
      try:
        ...
        # [NEW CANDLE DETECTION - ALWAYS ACTIVE]
        ...
        with lock:
            if state.get("last_m15_bar") is None:
                ...
                # Consume cooldown if active
                if state.get("cooldown_active", False):
                    state["cooldown_active"] = False
                    state["skip_this_candle"] = True
                    add_log("Cooldown active: Skipping entry window for this candle.", "warn")
                else:
                    state["skip_this_candle"] = False
        ...
        if b_active and entry_p is not None:
            ...
            for i, lot in enumerate(grid_lots):
                ...
                if hit:
                    res, f_price = send_order(direction, lot, f"GRID{i+1}")
                    if res:
                        tg_say(f"🔔 SAAK GRID ENTRY\nDirection: {direction}\nPrice: {f_price:.2f}\nLot: {lot}L\nLayer: {i+1}")
                        trig.add(i)
                        with lock: state["triggered"] = list(trig)
                        break  # FIX: Only one grid layer per tick to prevent burst-firing
            ...
            def liquidate(reason, is_profit):
                ...
                with lock:
                    ...
                    # If liquidating at the exact start of a candle, skip the entry window immediately
                    if new_candle_started:
                        state["skip_this_candle"] = True
                        state["cooldown_active"] = False
                        add_log("Trade exited at candle start: Skipping current entry window.", "warn")
                    else:
            ...
            # 3. Per-Basket Stop Loss (fixed, not cumulative daily)
            if pnl <= -DAILY_SL_USD:
                liquidate(f"BASKET SL (-${DAILY_SL_USD:.0f})", False)
                continue
            ...
            # 5. Level 3+ Breakeven & Trailing (n >= 3)
            with lock:
            ...
            if n >= 3:
                if not be_touched and pnl >= 5.0:
                    with lock:
                        ...
                        be_touched = True
                    add_log(f"Level {n} breakeven hit (+$5). Trailing -$100 and Candle-End exit active.", "warn")
                
                if be_touched:
                    if pnl <= -100.0:
                        liquidate(f"Level {n} TRAILING SL (-$100)", False)
                        continue
            ...
            if new_candle_started:
                if tp_latched and pnl > 0:
                    ...
                    continue
                if n >= 3 and be_touched and pnl >= 0:
                    liquidate(f"Level {n} CANDLE END EXIT", True)
                    continue
            


        # -- ENTRY LOGIC (Strict Candle Opening: First 60s of M15 Market Time) --
        if not running or b_active or state.get("skip_this_candle", False): continue
        ...
        res, fill_price = send_order(go_dir, FIXED_LOT, f"ENTRY-{pat}")
        if res:
            # Sanity check: reject if fill price is unreasonable (>50 from current mid)
            if abs(fill_price - mid) > 50:
                add_log(f"ENTRY REJECTED: fill_price {fill_price:.2f} too far from market {mid:.2f}", "error")
                close_all()  # Close the position we just opened
                continue
            with lock:
            ...
            add_log(f"NEW BASKET {go_dir} @ {fill_price:.2f} (Conf: {conf}%, Pat: {pat})", "tp")
...
@app.route("/state")
def api_state():
    # Read state snapshot under lock (fast)
    with lock:
        s = dict(state)
        s["bias_mode"] = BIAS_MODE
    # MT5 calls OUTSIDE lock to avoid blocking bot loop on live latency
    s["positions"] = []
    positions = mt5.positions_get(symbol=SYMBOL)
    if positions:
        tick = mt5.symbol_info_tick(SYMBOL)
